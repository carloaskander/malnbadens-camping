    ...
    return (
        <Paper sx={{ p: 3, mb: { xs: 2, lg: 0 }, width: 'fit-content', backgroundColor: 'rgba(0, 0, 0, 0.6)', borderLeft: '5px solid rgba(214, 107, 39)' }}>
            ...
            <Typography variant='h5' color='text.secondary' sx={{ mb: 1, fontSize: { xs: '16px', sm: '20px', md: '22px', lg: '25px' } }}>
                {t('bookingCountdown.season')} {currentTime.year()}: <Box component='span' sx={{ color: 'orange' }}>
                    {seasonOpenDate ? seasonOpenDate.format('D MMM') : t('bookingCountdown.loading')} - 
                    {seasonCloseDate ? seasonCloseDate.format('D MMM') : t('bookingCountdown.loading')}
                </Box>
            </Typography>
            <Button
                variant="contained"
